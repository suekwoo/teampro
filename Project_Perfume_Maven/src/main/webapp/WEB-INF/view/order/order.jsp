	...
	<div class="container">
		...
		<div class="container mt-3 address-input-div">
			...
			<input class="form-control" type="text" name="name" id="name"
				placeholder="받으시는 분의 성함을 입력해주세요." value="${member.name}"></div>		
			
			<div class="mt-3">
			<label>전화번호</label>
			<input class="form-control" type="text" name="tel" id="tel" value="${member.tel}"></div>
			...
			<input class="form-control" type="text" id="sample4_postcode"
				name="zipcode" placeholder="우편번호" readonly> </div>
			<div class="col">
		...
		<div class="product-div">
			...
			주문상품: <span class="kd-kind-span"></span>종 <span class="kd-quantity-span"></span>개
			</div>
			...
			<table class="table product-main-div" style="width: 80%;">
				...
				<tbody>
					<tr>
						...
						<td class="price-td">
							<fmt:formatNumber value='${p.price*quantityList[status.index]}' pattern="#,### 원" />
							<input type="hidden" name="multi-price" class="price-input" value="${p.price}">
							<input type="hidden" name="multi-prodnum" class="prodnum-input" value="${p.prodnum}">
							<input type="hidden" name="multi-quantity" class="quantity-input" value="${quantityList[status.index]}">
							<input type="hidden" name="multi-total-price" class="total-price-input" value="${p.price*quantityList[status.index]}">
						</td>
					</tr>
				</tbody>
			</table>
			...
			<table class="table product-main-div" style="width: 80%;">
				...
				<tbody>
					<tr>
						<td>${p.name}</td>
						<td>${quantity}</td>
						<td class="price-td-single">
							<fmt:formatNumber value='${p.price*quantity}' pattern="#,### 원" />
							<input type="hidden" name="single-price" class="single-price-input" value="${p.price}">
							<input type="hidden" name="single-prodnum" class="single-prodnum-input" value="${p.prodnum}">
							<input type="hidden" name="single-quantity" class="single-quantity-input" value="${quantity}">
							<input type="hidden" name="single-total-price" class="single-total-price-input" value="${p.price*quantity}">
						</td>
					</tr>
				</tbody>
			</table>
			...
			<div class="order-btn-div text-center">
				<input type="submit" formaction="${pageContext.request.contextPath}/order/orderPro"
