...
<body>
	...
	<div class="container">
		<div class="text-center mb-5"><h3>주문번호: ${ordernum}</h3>
		...
		<div class="d-flex justify-content-center">
			<table class="table table-hover orderList align-middle" style="width: 80%;">
				<thead>
					<tr>
				...
				<tbody>
					...
					<tr>
						<td class="prodname-td"><a href="${pageContext.request.contextPath}/product/productDetail?prodnum=${prodnumLi[status.index]}"
					...
					<tr>
						<td></td>
						<td><span>총: <c:out value="${totalQuantity}"/> 개</span></td>
						<td><span>합계: <fmt:formatNumber value='${totalPrice}' pattern="#,### 원" /></span></td>
					</tr>
					<tr>
						<td></td>
						<td><span>배송비: <fmt:formatNumber value='${order.delivercost}' pattern="#,### 원" /></span></td>
						<td><span>총액: <strong><fmt:formatNumber value='${totalPrice + order.delivercost}' pattern="#,### 원" /></strong></span></td>
					</tr>
				</tbody>
			</table>
		</div>
			...
			<c:choose>
			<c:when test="${order.result == 1}"> <!-- 결제 대기 상태 -->
			<button class="cancel-btn btn btn-outline-danger"
								data-ordernum="${order.ordernum}">주문취소</button></c:when>
			
			<c:when test="${order.result == 4}"> <!-- 배송 완료 -->
			...
			<!-- 취소 form -->
			<form action="${pageContext.request.contextPath}/order/orderCancelForm" 
