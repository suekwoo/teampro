...
<body>
	<div class="container">
		...
		<div class="d-flex justify-content-center">
			<table class="table table-hover orderList align-middle" style="width: 90%;">
				<thead>
					<tr>
					...
					<c:forEach var="o" items="${li}">
						<tr>
							<td class="ordernum-td">
								<input type="hidden" class="ordernum-input" value="${o.ordernum}">
								<a href="${pageContext.request.contextPath}/order/orderDetail?ordernum=${o.ordernum}"
								style="color: black;'">${o.ordernum}</a>
							</td>
							
							<c:choose>
								<c:when test="${o.result == 1}"> <!-- 주문 완료 -->
								<td><span>주문 완료</span></td>
								<td colspan="2"><button class="cancel-btn btn btn-outline-danger"
								data-ordernum="${o.ordernum}">주문취소</button></td> </c:when>
								
								<c:when test="${o.result == 2}"> <!-- 배송 중 -->
								<td><span>배송 중</span></td>
								<td colspan="2"><span>배송 중인 상품은 취소 하실 수 없습니다.</span></td></c:when>
								
								<c:when test="${o.result == 3}"> <!-- 주문 취소 -->
								<td><span style="color: crimson;">주문 취소</span></td>
								<td colspan="2"></c:when>
								
								<c:when test="${o.result == 4}"> <!-- 배송 완료 -->
								<td><span>배송 중</span></td>
								<td colspan="2"><button class="confirm-btn btn btn-outline-primary"
								data-ordernum="${o.ordernum}">주문확정</button></td> </c:when>
								
								<c:when test="${o.result == 5}"> <!-- 주문 확정 -->
